...
A responsive Puck-native paragraph with body, lead, muted, and small styles.

## Install

```sh
pnpm add @anvilkit/text @puckeditor/core
```
...
export function Example() {
  return <Text text="Clear supporting copy." variant="lead" />;
}
```
...
the `text.targets.<targetId>` label in all four catalogs.
