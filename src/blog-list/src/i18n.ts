...
import type { BlogListPost } from "./BlogList";

export type BlogListMessageKey = keyof typeof en;

const PACKS: Readonly<Record<string, Record<string, string>>> = { ja, ko, zh };

/**
 * PLAN-0027 §2.3 — host-injected external adapter for the `posts`
 ...
 */
export interface BlogListPostsAdapter {
	fetchList: () => Promise<unknown[]>;
	mapItem?: (item: unknown) => BlogListPost;
...
export const blogListI18nEntry = {
	namespace: "blog-list",
