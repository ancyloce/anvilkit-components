...
import type { PricingPlan } from "./PricingMinimal";

export type PricingMinimalMessageKey = keyof typeof en;

const PACKS: Readonly<Record<string, Record<string, string>>> = { ja, ko, zh };

/**
 * PLAN-0027 §2.3 — host-injected external adapter for the `plans`
 ...
 */
export interface PricingMinimalPlansAdapter {
	fetchList: () => Promise<unknown[]>;
	mapItem?: (item: unknown) => PricingPlan;
...
export interface CreateComponentConfigOptions {
	...
	dataSources?: {
		plans?: PricingMinimalPlansAdapter;
	};
}
...
): (key: PricingMinimalMessageKey) => string {
	const pack =
...
export const pricingMinimalI18nEntry = {
	namespace: "pricing-minimal",
