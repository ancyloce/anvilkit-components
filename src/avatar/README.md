...
A Puck-native avatar component scaffold generated for `anvilkit-components`.

## Install

```sh
pnpm add @anvilkit/avatar @puckeditor/core
```
...
export function Example() {
  return <Avatar title="Avatar" body="<p>Start editing this block.</p>" />;
}
```
...
the `avatar.targets.<targetId>` label in all four catalogs.
