...
```sh
pnpm add @anvilkit/textarea @anvilkit/ui @puckeditor/core
```
...
export function Example() {
  return <Textarea label="Textarea" name="textarea" placeholder="Enter a value" />;
}
```
...
the `textarea.targets.<targetId>` label in all four catalogs.
