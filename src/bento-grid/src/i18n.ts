...
export type BentoGridMessageKey = keyof typeof en;
 ...
 * collection (the grid's cards). Functions live in config
 ...
 */
export interface BentoGridItemsAdapter {
	fetchList: () => Promise<unknown[]>;
	mapItem?: (item: unknown) => BentoGridItem;
...
): (key: BentoGridMessageKey) => string {
	const pack =
...
export const bentoGridI18nEntry = {
	namespace: "bento-grid",
