...
```sh
pnpm add @anvilkit/select @anvilkit/ui @puckeditor/core
```
...
export function Example() {
  return <Select label="Select" name="select" placeholder="Enter a value" />;
}
```
...
the `select.targets.<targetId>` label in all four catalogs.
