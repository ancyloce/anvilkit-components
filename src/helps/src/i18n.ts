...
import type { HelpsAvatar } from "./Helps";

export type HelpsMessageKey = keyof typeof en;

const PACKS: Readonly<Record<string, Record<string, string>>> = { ja, ko, zh };

/**
 * PLAN-0027 §2.3 — host-injected external adapter for the `avatars`
 ...
 */
export interface HelpsAvatarsAdapter {
	fetchList: () => Promise<unknown[]>;
	mapItem?: (item: unknown) => HelpsAvatar;
