...
A Puck-native table component scaffold generated for `anvilkit-components`.

## Install

```sh
pnpm add @anvilkit/table @puckeditor/core
```
...
export function Example() {
  return <Table title="Table" body="<p>Start editing this block.</p>" />;
}
```
...
the `table.targets.<targetId>` label in all four catalogs.
