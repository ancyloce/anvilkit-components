...
A Puck-native alert component scaffold generated for `anvilkit-components`.

## Install

```sh
pnpm add @anvilkit/alert @puckeditor/core
```
...
export function Example() {
  return <Alert title="Alert" body="<p>Start editing this block.</p>" />;
}
```
...
the `alert.targets.<targetId>` label in all four catalogs.
