...
A Puck-native badge component scaffold generated for `anvilkit-components`.

## Install

```sh
pnpm add @anvilkit/badge @puckeditor/core
```
...
export function Example() {
  return <Badge title="Badge" body="<p>Start editing this block.</p>" />;
}
```
...
the `badge.targets.<targetId>` label in all four catalogs.
