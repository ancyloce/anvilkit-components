...
A Puck-native progress component scaffold generated for `anvilkit-components`.

## Install

```sh
pnpm add @anvilkit/progress @puckeditor/core
```
...
export function Example() {
  return <Progress title="Progress" body="<p>Start editing this block.</p>" />;
}
```
...
the `progress.targets.<targetId>` label in all four catalogs.
