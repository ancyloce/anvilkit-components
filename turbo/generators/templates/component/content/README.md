...
A Puck-native {{name}} component scaffold generated for `anvilkit-components`.
...
export function Example() {
  return <{{componentName}} title="{{componentLabel}}" body="<p>Start editing this block.</p>" />;
}
```
...
the `{{name}}.targets.<targetId>` label in all four catalogs.
